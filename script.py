...
# Filtro
funds = []
maximo = 50  # len(allTicker)
for i in range(0, maximo):
    url = "https://fiis.com.br/" + allTicker[i] + "/"
    driver.get(url)
    statement = [allTicker[i]]
    valor_patrimonial = driver.find_element(by=By.XPATH,
                                            value="//*[@id=\"carbon_fields_fiis_quotations_chart-2\"]/div[2]/div[3]/p[1]/b").get_attribute(
        'innerText').replace('.', '').replace(',', '.')
    statement.append(float(valor_patrimonial))
    cotacao = driver.find_element(by=By.XPATH,
                                  value="//*[@id=\"carbon_fields_fiis_quotations_chart-2\"]/div[1]/div[2]/div[1]/div[1]/span[2]").get_attribute(
        "innerText").replace('.', '').replace(',', '.')
    statement.append(float(cotacao))
    patrimonio_liquido = driver.find_element(by=By.XPATH,
                                             value="//*[@id=\"carbon_fields_fiis_header-2\"]/div/div/div[1]/div[2]/div/div[3]/p[1]/b").get_attribute(
        "innerText")
    statement.append(patrimonio_liquido)
    rendimento = driver.find_element(by=By.XPATH,
                                     value="//*[@id=\"carbon_fields_fiis_header-2\"]/div/div/div[1]/div[2]/div/div[2]/p[1]/b").get_attribute(
        "innerText").replace('.', '').replace(',', '.')
    statement.append(float(rendimento))
    dividend_yield = driver.find_element(By.XPATH,
                                         "//*[@id=\"carbon_fields_fiis_header-2\"]/div/div/div[1]/div[2]/div/div[1]/p[1]/b").get_attribute(
        "innerText").replace('.', '').replace(',', '.')
...
dataframe = pd.DataFrame(funds, columns=['Ticker', 'VP', 'COT', 'PL', 'REND', 'DY'])
print(dataframe)
dataframe.to_csv('FII.csv')
